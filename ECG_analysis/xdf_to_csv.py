...
def export_xdf_hmd_synced(xdf_path: Path, out_dir: Path):
    """
    Export all streams from an XDF file, using the HMD stream as the global
    time reference. For each continuous signal stream, write a CSV with:

        time  (float, seconds since HMD start)
        ch1, ch2, ...

    For event streams, write a CSV with:

        onset     (seconds since HMD start)
        sample    (HMD sample index)
        type
        value
        offset
        duration
    ...
    manifest = {
        ...
        "id": xdf_path.stem,
        "greenery": xdf_path.stem[5] if len(xdf_path.stem) > 5 else None,
    ...
    for k, stream in enumerate(streams):
        ...
        if data.ndim == 1:
            ...
            # shape (n_channels, n_samples) -> transpose
            data = data.T
        ...
        # Build DataFrame with explicit time column
        # time = seconds since HMD start, aligned with HMD timeline
        arr = np.column_stack([t_rel, y])
        df = pd.DataFrame(arr, columns=["time"] + headers)
        ...
        sidecar = {
            ...
            "reference": {
                ...
                "timeline": "HMD",
                "time_column": "time",
                "time_unit": "seconds_since_HMD_start",
                "coverage_note": "Samples outside the stream’s original coverage are not present in the CSV.",
            },
            "columns": list(df.columns),
        }
        with open(json_path, "w", encoding="utf-8") as f:
...
def main():
    if __name__ == "__main__":
        in_dir = Path(r"F:\BIOSTAT_raw_data\0_source_data")
        out_dir = Path(r"F:\BIOSTAT_raw_data\1_raw_data_Magda")
